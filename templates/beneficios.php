<!-- Sección - Proveemos los mejores beneficios -->
<div class="container-fluid pt-4 pb-5 bg-gray px-0">
  <div class="container pb-5">
    <div class="row text-center py-5">
      <h2 class="text-primary fs-sm-24 fw-bold">Desarrollamos soluciones innovadoras para las cooperativas líderes en Puerto Rico</h2>
    </div>
    <div id="mobile-side-scroll" class="row">
      <div class="col-12 col-md-3">
        <div class="p-4 bg-white rounded-3 h-100 filter-white">
          <img class="mb-3" src="<?php $SERVER_URI ?>images/experiencia.svg" alt="">
          <h3 class="fs-6 text-primary text-bold">Experiencia</h3>
          <p>Tenemos una amplia experiencia en el mercado. 38 años sirviendo de manera exclusiva al mercado de ahorro y crédito.</p>
        </div>
      </div>
      <div class="col-12 col-md-3">
        <div class="p-4 bg-white rounded-3 h-100 filter-white">
          <img class="mb-3" src="<?php $SERVER_URI ?>images/unico-proveedor.svg" alt="">
          <h3 class="fs-6 text-primary text-bold">Centro de Servicio Local</h3>
          <p>Único proveedor para las cooperativas con un centro de llamadas de servicio al cliente en Puerto Rico.</p>
        </div>
      </div>
      <div class="col-12 col-md-3">
        <div class="p-4 bg-white rounded-3 h-100 filter-white">
          <img class="mb-3" src="<?php $SERVER_URI ?>images/cero-desventajas.svg" alt="">
          <h3 class="fs-6 text-primary text-bold">Continuo Desarrollo</h3>
          <p>Única institución de servicios que se ha mantenido desarrollando y educando al personal.</p>
        </div>
      </div>
      <div class="col-12 col-md-3">
        <div class="p-4 bg-white rounded-3 h-100 filter-white">
          <img class="mb-3" src="<?php $SERVER_URI ?>images/mantenimiento.svg" alt="">
          <h3 class="fs-6 text-primary text-bold">Mantenimiento</h3>
          <p>Realizar las mejoras al sistema a través de conexión remota para agilizar los procesos de los clientes.</p>
        </div>
      </div>
    </div>
  </div>
</div>